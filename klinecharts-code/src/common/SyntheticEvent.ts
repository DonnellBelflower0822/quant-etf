...
export default class SyntheticEvent {
  ...
  private _clickCoordinate: Coordinate = {
  ...
  private _tapCoordinate: Coordinate = {
    x: Number.NEGATIVE_INFINITY,
    y: Number.POSITIVE_INFINITY,
  };
  ...
  private _touchMoveHandler(moveEvent: TouchEvent): void {
    ...
    if (
      ...
      manhattanDistance < ManhattanDistance.CancelTap
    ) {
      return;
    }
    ...
    if (!this._touchMoveExceededManhattanDistance) {
      ...
      const isVertDrag =
        yOffset >= correctedXOffset &&
        !this._options.treatVertDragAsPageScroll();
      const isHorzDrag =
        correctedXOffset > yOffset &&
  ...
  private _mouseDownHandler(downEvent: MouseEvent): void {
    ...
    {
      const boundMouseMoveWithDownHandler =
  ...
  private _init(): void {
...

    {
      ...
      const outsideHandler = (event: MouseEvent | TouchEvent): void => {
        ...
        if (
          event.composed &&
          this._target.contains(event.composedPath()[0] as Element)
        ) {
          return;
        }
        ...
        if (
          event.target !== null &&
          this._target.contains(event.target as Element)
        ) {
          return;
        }
  ...
  private _initPinch(): void {
    if (
      !isValid(this._handler.pinchStartEvent) &&
  ...
  private _firesTouchEvents(e: MouseEvent): boolean {
    ...
    return (
      this._eventTimeStamp(e) <
      this._lastTouchEventTimeStamp + Delay.PreventFiresTouchEvents
    );
  }
