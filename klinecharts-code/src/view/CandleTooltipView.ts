...
export default class CandleTooltipView extends IndicatorTooltipView {
  ...
  private _drawCandleStandardTooltip(
    ...
    if (this.isDrawTooltip(crosshair, tooltipStyles)) {
      ...
      const values = this._getCandleTooltipData(
        {
          prev: dataList[dataIndex - 1] ?? null,
          current: crosshair.kLineData!,
      ...
      const [
        leftIconsNextStartX,
        leftIconsNextStartY,
        leftIconsLastRowHeight,
        leftIconsIncreaseHeight,
      ] = this.drawStandardTooltipIcons(
        ctx,
        bounding,
        { paneId, indicatorName: "", iconId: "" },
        activeTooltipIcon,
        leftIcons,
        x,
        y,
      ...
      const [
        middleIconsNextStartX,
        middleIconsNextStartY,
        middleIconsLastRowHeight,
        middleIconsIncreaseHeight,
      ] = this.drawStandardTooltipIcons(
        ctx,
        bounding,
        { paneId, indicatorName: "", iconId: "" },
      ...
      if (values.length > 0) {
        const [
          valuesNextStartX,
          valuesIconsNextStartY,
          valuesIconsLastRowHeight,
          valuesIconsIncreaseHeight,
        ] = this.drawStandardTooltipLabels(
          ctx,
          bounding,
          values,
          x,
      ...
      const [
        rightIconsNextStartX,
        rightIconsNextStartY,
        rightIconsLastRowHeight,
        rightIconsIncreaseHeight,
      ] = this.drawStandardTooltipIcons(
        ctx,
        bounding,
        { paneId, indicatorName: "", iconId: "" },
  ...
  private _drawRectTooltip(
    ...
    if (isDrawCandleTooltip || isDrawIndicatorTooltip) {
      ...
      if (isDrawCandleTooltip) {
        ...
        rectHeight +=
          (baseTextMarginBottom + baseTextMarginTop + baseTextSize) *
          candleTooltipDatas.length;
      }
      ...
      if (rectWidth !== 0 && rectHeight !== 0) {
        ...
        const isPointer =
          rectPosition === CandleTooltipRectPosition.Pointer &&
        ...
        if (isPointer) {
          ...
          if (isLeft) {
            ...
            if (
              styles.yAxis.inside &&
              styles.yAxis.position === YAxisPosition.Right
            ) {
              rectX -= yAxisBounding.width;
            }
          }
        ...
        const candleTextX =
          rectX + rectBorderSize + rectPaddingLeft + baseTextMarginLeft;
        ...
        if (isDrawIndicatorTooltip) {
          ...
          const indicatorTextX =
            rectX + rectBorderSize + rectPaddingLeft + indicatorTextMarginLeft;
  ...
  private _getCandleTooltipData(
    _data: CandleTooltipCustomCallbackData,
    ...
    // const mapping = {
    //   "{time}": customApi.formatDate(
    //     dateTimeFormat,
    //     current.timestamp,
    //     "YYYY-MM-DD HH:mm",
    //     FormatDateType.Tooltip
    //   ),
    //   "{open}": formatThousands(
    //     formatPrecision(current.open, pricePrecision),
    //     thousandsSeparator
    //   ),
    //   "{high}": formatThousands(
    ...
    //   "{change}":
    //     prevClose === 0
    //       ? tooltipStyles.defaultValue
    //       : `${formatPrecision((changeValue / prevClose) * 100)}%`,
    // };
    // const labelValues = (isFunction(tooltipStyles.custom)
