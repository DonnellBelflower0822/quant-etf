...
export default class IndicatorTooltipView extends View<YAxis> {
  private readonly _boundIconClickEvent =
    (currentIcon: TooltipIcon, iconId: string) => () => {
      const pane = this.getWidget().getPane();
      pane
        .getChart()
        .getChartStore()
        .getActionStore()
        .execute(ActionType.OnTooltipIconClick, { ...currentIcon, iconId });
      return true;
    };

  private readonly _boundIconMouseMoveEvent =
    (currentIconInfo: TooltipIcon, iconId: string) => () => {
      const pane = this.getWidget().getPane();
  ...
  protected drawIndicatorTooltip(
    ...
    if (this.isDrawTooltip(crosshair, tooltipStyles)) {
      ...
      indicators.forEach((indicator) => {
        const { name, calcParamsText, values, icons } =
        ...
        if (nameValid || valuesValid) {
          const [leftIcons, middleIcons, rightIcons] =
            this.classifyTooltipIcons(icons);
          ...
          const [
            ...
            ctx,
            bounding,
            { paneId, indicatorName: indicator.name, iconId: "" },
            activeTooltipIconInfo,
            leftIcons,
            x,
            y,
            prevRowHeight
  ...
  protected drawStandardTooltipIcons(
    ...
    if (icons.length > 0) {
      ...
      icons.forEach((icon) => {
        ...
        x += marginLeft;
        const active =
          activeIcon?.paneId === currentIcon.paneId &&
          activeIcon?.indicatorName === currentIcon.indicatorName &&
          activeIcon?.iconId === icon.id;
        this.createFigure(
          {
            name: "text",
  ...
  protected isDrawTooltip(
    _crosshair: Crosshair,
    _styles: TooltipStyle
  ): boolean {
    console.log(111);
    return false;
  ...
  protected getIndicatorTooltipData(
    ...
    const tooltipData: IndicatorTooltipData = {
      name,
      calcParamsText,
      values: [],
      icons: tooltipStyles.icons,
    };
    ...
    if (indicator.createTooltipDataSource !== null) {
      ...
      const {
        name: customName,
        calcParamsText: customCalcParamsText,
        values: customValues,
        icons: customIcons,
      } = indicator.createTooltipDataSource({
