...
export type IndicatorFigureStyle = Partial<Omit<SmoothLineStyle, "style">> &
  Partial<Omit<RectStyle, "style">> &
  Partial<TextStyle> &
  Partial<{
...
export type IndicatorFigureAttrs = Partial<ArcAttrs> &
  Partial<LineStyle> &
  Partial<RectAttrs> &
  Partial<TextAttrs> &
  Record<string, any>;
...
export type IndicatorFigureAttrsCallbackCoordinate<D> =
  IndicatorFigureCallbackBrother<Record<keyof D, number> & { x: number }>;
...
export type IndicatorFigureStylesCallbackData<D> =
...
export type IndicatorTemplate<D = any> = ExcludePickPartial<
  Omit<Indicator<D>, "result">,
  "name" | "calc"
>;

export type IndicatorCreate<D = any> = ExcludePickPartial<
  Omit<Indicator<D>, "result">,
  "name"
>;
...
export default abstract class IndicatorImp<D = any> implements Indicator<D> {
  ...
  setPrecision(precision: number, flag?: boolean): boolean {
    ...
    if (
      optimalPrecision !== this.precision &&
      precision >= 0 &&
  ...
  setRegenerateFigures(
    callback: Nullable<IndicatorRegenerateFiguresCallback>
  ): boolean {
  ...
  setCreateTooltipDataSource(
    callback: Nullable<IndicatorCreateTooltipDataSourceCallback>
  ): boolean {
